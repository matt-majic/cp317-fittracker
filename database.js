...
export async function getTrainee(id) {
  try {
    // Fetch data from the `trainees` table
    ...
    if (!trainee[0]) {
      return null; // Return null if no trainee is found
    }
...
export async function getTrainer(id) {
  try {
    const result = await query(`SELECT * FROM trainers WHERE user_id = ?`, [
      id,
    ]);
    return result[0];
  } catch (error) {
...
export async function getCalorieTrackers(id) {
  try {
    const calorieTracker = await query(
      "SELECT * FROM calorie_trackers WHERE user_id = ?",
      [id]
    );
...
export async function createCalorieTracker(calorieTracker) {
  try {
    await query(
      ...
      [
        calorieTracker["user_id"],
        calorieTracker["date"],
        calorieTracker["calorieIn"],
        calorieTracker["calorieOut"],
        calorieTracker["calorieGoal"],
      ]
    );
    return 200;
  } catch (error) {
...
export async function addFoodItem(food) {
  try {
    await query("INSERT INTO food_items (name, calories) VALUES (?, ?)", [
      food["name"],
      food["calories"],
    ]);
    return 200;
...
export async function removeFoodItem(foodId) {
  try {
    await query("DELETE FROM food_items WHERE id = ?", [foodId]);
    return 200;
  } catch (error) {
...
export async function savePaymentMethod(traineeId, paymentMethod) {
  try {
    const { issuer, cardHolderName, cardNum, expiration, cvv, billingAddress } =
      paymentMethod;
...
export async function deletePaymentMethod(traineeId) {
  try {
    await query("DELETE FROM payment_methods WHERE user_id = ?", [traineeId]);
    return 200;
  } catch (error) {
